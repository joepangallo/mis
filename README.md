...
| 7 | [`module-07-enhancing-organizational-decision-making.html`](module-07-enhancing-organizational-decision-making.html) | Chapter 7 Objectives 7.1&ndash;7.4, built as the companion to the Chapter 7 learning game and running on the same invented firm: descriptive, diagnostic, predictive and prescriptive analytics, and which of them recommends an action rather than informing a person &middot; the decision-making continuum, and why it describes the decision rather than the seniority of whoever makes it &middot; decision support, measures against dimensions, and slicing, dicing, drilling down and rolling up on a real cube &middot; push, exception and pull reporting, dashboards, and what-if, sensitivity, goal seeking and optimisation &middot; supervised against unsupervised learning, association rules with computed support and confidence, clustering and anomaly detection, and what a cluster does not tell you &middot; the machine learning workflow end to end, why accuracy on the data a model was fitted to proves nothing, and why the threshold is a business decision &middot; generative AI taught as a **cost and control ladder** rather than a product comparison, with hallucination treated as a property of how these systems work &middot; agents, where a person stands in the loop, governance, and synthetic media &middot; explicit against tacit knowledge, and what leaves the building when one person does &middot; geographic information systems, map layers, and why a raw count mostly shows you where the people are &middot; plus two clearly labeled application supplements: **the five forces and the value chain**, and one scoped analytics recommendation. Twelve sections, 36 interactive activities, 70 glossary terms, and a 42-question final challenge. **No vendor, model name or version appears anywhere** &mdash; capability is taught by use case, because any such comparison is stale within a term. HTML edition; no PDF companion generated yet. |
...
node src/build.mjs --module=modules/07  # regenerate Module 7
node src/check.mjs --module=modules/07  # the same checks against Module 7
